...
void parse_ambient(t_pworld *pworld, char *line)
{
	...
	pworld->A.ratio = parse_number(toks[1]);
	if (pworld->A.ratio > 1.0 || pworld->A.ratio < 0.0)
		parse_error(EXIT_FAILURE, "Ratio must be 0.0 - 1.0");
	pworld->A.a_color = parse_color(toks[2]);
	pworld->A.is_set = true;
	free_split(toks);
